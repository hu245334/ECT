...
if __name__ == "__main__":
    args = get_args()
    with open("config/ECT_train.yaml") as cfg:
        config = yaml.safe_load(cfg)
    blur_path = "./datasets/Realblur_R/test/blur/"
    out_path = "./out/ECT_Realblur_R"
    ...
    for file in os.listdir(blur_path):
        ...
        for img_name in os.listdir(blur_path + "/" + file):
            ...
            img_tensor = (
                torch.from_numpy(np.transpose(img / 255, (2, 0, 1)).astype("float32"))
                - 0.5
            )
            with torch.no_grad():
    ...
    for file in os.listdir(blur_path):
        ...
        for img_name in os.listdir(blur_path + "/" + file):
            ...
            with torch.no_grad():
                ...
                print(
                    "Image:{}/{}, CNN Runtime:{:.4f}".format(
                        iteration, total_image_number, (stop - start)
                    )
                )
